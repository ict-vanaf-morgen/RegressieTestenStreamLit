...
def highlight_current_row_bold(x):
    # Zorg dat alleen de actieve rij een dikgedrukt wordt
    return ['font-weight: bold' if x.name == st.session_state.step_index else '' for _ in x]
...
if uploaded_file:
    try:
        ...
        # Details geselecteerde stap en teller
        st.write(f"Huidige stap: {step_num + 1} / {len(df)}")
        ...
        # Toon dataframe met actieve rij dikgedrukt
        st.dataframe(df.style.apply(highlight_current_row_bold, axis=1))

        # Slider navigatie onder de tabel
