...
if uploaded_file:
    try:
        ...
        # Veilige initialisatie - app blijft werken zelfs met lege testcases
        if not testcase_ids:
            st.warning("⚠️ Geen testcases gevonden in het JSON-bestand!")
            st.info("Het bestand bevat wel metadata, maar geen testcases. Je kunt de metadata bekijken via de sidebar.")
            
            # Toon alleen metadata en stop verdere verwerking van testcases
            if st.session_state.show_metadata:
                st.sidebar.markdown("### Metadata inhoud:")
                metadata = data.get("metadata", {})
                for key, value in metadata.items():
            ...
            # Stop hier de verdere uitvoering van testcase-gerelateerde code
            st.stop()

        # Als er wel testcases zijn, ga verder met normale verwerking
        if st.session_state.selected_tc_id is None or st.session_state.selected_tc_id not in testcase_ids:
        ...
        with col_buttons:
            # Maak 7 kolommen voor de buttons
            ...
            if btn_f.button("f ❌", key="btn_f_fault"):
                update_status(df.iloc[st.session_state.step_index]["step"], "fout❌")
                st.rerun()
            if btn_colon.button("t 📝", key="btn_colon_comment"):
        ...
        st.info("Probeer een ander JSON-bestand of controleer de structuur van het bestand.")
else:
